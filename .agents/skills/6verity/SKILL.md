...
华为杯论文（模板 `huaweibei` / `huaweibei-latex`，或 `plan.md` 中竞赛类型为华为杯）额外读取 `../_references/huaweibei_excellent_paper_patterns.md` §1 与 §6，执行下文 Step 6b。
...
- **AI 工具使用声明**：参考文献中存在 `工具名称, 版本/型号, 开发机构, 使用日期` 格式的条目，且与 `reports/AI_USAGE.md`（`hwb-kickoff` 产出）登记的工具一致；登记为 AI 生成的段落/代码在正文或附录对应位置有标注。缺失判 `FAIL`。
- **摘要**：编译后摘要页 ≤ 2 页；每个子问题在摘要中有对应段落（“针对问题 N”）。缺小问判 `FAIL`；段内无任何数值或明确定性结论判 `WARN`。
- **公式**：正文无公式截图（图片文件名或 caption 含 `eq`/`formula`/`公式` 且无对应数学环境时人工核查）。判 `FAIL`。
- **验证证据**：对 `ANALYSIS_MODELING_REPORT.md` 列出的每个主模型，在论文中定位至少一处误差/基线对比/扰动或灵敏度/约束满足性的证据（可位于该问的结果分析小节，不要求独立章）。缺失判 `FAIL`。

其余（小结、单位列、关键词数、模型评价具体性、附录文件清单、篇幅提示、重述照抄）为 `WARN`。检查结果写入 `reports/VERIFY_REPORT.md` 的“华为杯对标”小节，逐项列出位置（文件 + 标题）而不是只写通过/不通过。
...
```bash
# 在项目根目录执行；--root 让 sections/ 内的 ../../figures/ 相对路径能被 Typst 沙箱访问
command -v typst >/dev/null 2>&1 && typst compile --root . "$MAIN_FILE" "$OUTPUT_PDF"
```

**LaTeX 编译**：

```bash
# 在 paper/ 目录执行；LaTeX 中 \input 的章节里图片路径相对于 main.tex 所在目录（../figures/），与 Typst 不同
...
- 华为杯：缺 AI 工具使用声明、摘要超两页或缺小问、公式截图、主模型无任何验证证据、队号/学校出现在封面以外。
...
- 代码完整复现耗时过长，只做了轻量检查。
- 华为杯对标清单中标为 WARN 的项（见 `huaweibei_excellent_paper_patterns.md` §6）。
