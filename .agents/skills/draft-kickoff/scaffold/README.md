...
> 数学建模论文项目（由 mm-draft-workbench 生成，{{DATE}}）。
> 这是一个**自包含、可复现**的项目文件夹：论文正文（`paper/sections/*.md`）、建模报告、数据与模型代码、结果、图都在这里；
> Windows / Linux / macOS 上只要有 Python 3.10+ 就能重跑所有数据处理、模型、图，并对论文章节做自检。
...
| 想要 | 位置 | 说明 |
| --- | --- | --- |
| 论文正文 | `paper/sections/*.md` + `paper/paper.yaml` | **一章一文件**，按提交质量写完的终稿；`paper.yaml` 存题目 / 关键词 / 章节顺序（见 `paper/README.md`） |
| 论文章节自检 | `reports/PAPER_CHECK.md` | `python run_all.py paper` 重建：交叉引用、图路径、占位符、内部名泄露、图/表/公式编号表 |
...
- 论文章节自检只查结构与引用，不查数字是否与 `reports/RESULTS_REPORT.md` 一致，交稿前需逐数核对。
