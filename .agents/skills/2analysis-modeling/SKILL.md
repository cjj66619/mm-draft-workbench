...
- 代码实现要点。
- 结果校验方法。
- 基线方案：一个可在数小时内实现的简单对照（经验公式、朴素统计、线性/单目标简化等），主模型的改进必须相对它可量化。
- 验证证据计划：至少一种——误差指标（含数据集/划分方式）、与基线对比、参数扰动或灵敏度、约束满足性检查——并写清将在论文哪一节呈现。
- 与前后问的接口：本问用到前一问的哪个输出，向下一问交付什么。

华为杯优秀论文的共同点是各小问递进（前一问的结论成为后一问的输入）、每问有对照和验证（见 `../_references/huaweibei_excellent_paper_patterns.md` §2、§4）；官方模板明确“模型应正确、简明、可解、实用，不应为了数学难度而复杂化”，选复杂模型必须写出为什么简单模型不够。
