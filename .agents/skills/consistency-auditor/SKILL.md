...
本 skill 来自 zhnnky329/MathModeling-skills（MIT），在 `6verity` 之前或之中以 `final` 模式调用，做跨媒介一致性审计。事实来源映射：

- 论文数值的唯一来源：`reports/RESULTS_REPORT.md`（对应原文的 `frozen_numbers.json`）；
- 符号与单位：`reports/ANALYSIS_MODELING_REPORT.md` 的符号说明表（对应 `planning/symbol_table.md`）；
- 人的建模决策：`plan.md` / `todo.md` 与对话记录（对应 `qx_decisions.jsonl`）；
- 论文：`paper/`；图：`figures/`；代码：`code/`。
- 审计结果写到 `reports/audits/cross_media_consistency_audit.md`。
