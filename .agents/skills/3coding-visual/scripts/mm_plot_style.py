...
def _write_figure_readme(folder: Path, stem: str, info: dict, docstring: str, review: dict) -> None:
    ...
    if readme.exists():
        ...
        if _README_MANUAL_MARK in old:
            manual = old.split(_README_MANUAL_MARK, 1)[1].strip()
    lines = [f"# {stem}", ""]
    lines += ["> 本文件由 `mm_plot_style.save_fig` 自动生成，`## 人工备注` 之后的内容会被保留。", ""]
...
def save_fig(
    ...
    *,
    ...
    if layout == "folder":
        for s in sources:
            s = Path(s)
            snap = snapshot_data(s, folder) if (s.exists() or (folder / s.name).exists()) else None
            data_records.append({
                "source": _SNAPSHOTS.get(str(s.resolve()), _relpath(s)),
