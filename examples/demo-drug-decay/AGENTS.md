...
- 目标平台：Windows / Linux / macOS，**只依赖 Python 3.10+**（`requirements.txt`），不依赖 bash、draw.io CLI、pandoc、LibreOffice。
- 入口只有两个：`python doctor.py`（体检）、`python run_all.py [code|figures|figcheck|check|<fig_id>]`（复现）。
...
| 内容 | 真源 | 派生物 |
| --- | --- | --- |
| 论文正文 | `paper/main.docx`（已冻结，见 `paper/DOCX_FREEZE.json`） | `paper/sections/*.md` 是**历史生成源**，只作参考，不再重新生成 Word |
| 所有数值 | `results/*.csv|json` → 汇总在 `reports/RESULTS_REPORT.md` | Word 里的数字、图上的标注 |
| 模型定义 | `reports/ANALYSIS_MODELING_REPORT.md` §模型 + `code/1x_*.py` | Word 的模型章节 |
...
2. 改了代码 → 必须重跑 `python run_all.py code figures` → 更新 `reports/RESULTS_REPORT.md` → 再改 Word。不允许只改 Word 里的数字。
3. Word 是唯一正文真源：文字润色、排版、图表编号在 Word 里做。**不要**运行任何"重新生成 docx"的操作覆盖它；`paper/sections/*.md` 不再维护。
4. 所有随机过程固定种子（`common.set_seed(42)`），能从干净状态一遍跑通。
5. 论文正文中不得出现内部路径/工作流名（`reports/`、`figures/`、`AGENTS.md`、`plan.md`、`todo.md`、`run_all`）。
...
| 任务 | 步骤 |
| --- | --- |
| 排版 / 润色 | 直接改 `paper/main.docx`；改动数字要回到 §1 规则 2 |
| 绘图优化 | `python run_all.py figcheck` → 看 `figures/FIGURE_REVIEW.md` → 改 `make_figure.py` → `python run_all.py figures <fig_id>` → 看 PNG → 更新 Word 里的图 |
| 补实验 | 新增 `code/3x_*.py` 与 `figures/figNN_*/make_figure.py`（用 `_template_figure/` 起手）→ 重跑 → `RESULTS_REPORT.md` 加节 → Word 加内容 → `todo.md`/`HANDOFF.md` 更新 |
| 换数据 | 只改 `data/raw/`（新增文件不覆盖旧文件）与 `code/00_*.py` → 全量重跑 → 比对 `RESULTS_REPORT.md` 差异 |
| 交接 | 更新 `HANDOFF.md`（做了什么、验证了什么、剩什么、坑在哪）与 `todo.md` |

## 5. 禁止

- 覆盖 `paper/main.docx`（除非人明确要求，且先备份）。
- 修改 `data/raw/`。
- 在代码里写绝对路径、调用 bash/sh、`subprocess.run(["python", ...])`。
- 手工编辑自动生成文件：`figures/*/manifest.json`、`review.json`、`figures/README.md`、`figures/FIGURE_REVIEW.md`、`reports/RUN_STATUS.md`。
