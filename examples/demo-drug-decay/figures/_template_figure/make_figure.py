...
2. 每个图元对应的机理
   ...
   - 阴影：拟合曲线的 95% 置信带，由 data.csv 的 ci_low / ci_high 列给出（模型代码用 delta 法算好，绘图不重算）；
     只表征参数不确定性，不含观测噪声。若上游没有这两列则不画阴影
...
3. 输入数据每列的含义与来源
   ...
   可选 ci_low / ci_high（拟合值 95% 置信带上下界）
4. 参数与随机种子
...
def main() -> None:
    ...
    if {"ci_low", "ci_high"} <= set(df.columns):
        ax1.fill_between(df["t"], df["ci_low"], df["ci_high"], color="C1", alpha=0.2, lw=0, label="95% 置信带")
